...
id_genes <- unname(unlist(read.csv("/n/groups/walsh/indData/Maya/Finalized_Convergence_In_ASD/Cell_Lines/figures/sfari_enrichment_barplot/intellectual_disability_sysNDD_list.csv", header = FALSE)))
just_id_genes <- setdiff(id_genes, SFARI_genes)
just_SFARI_genes <- setdiff(SFARI_genes, id_genes)
just_high_conf_SFARI_genes <- setdiff(high_conf_SFARI_genes, id_genes)

consensus_asd <- readRDS("/n/groups/walsh/indData/Maya/Finalized_Convergence_In_ASD/Consensus_ASD/raw_data/database/gene_lists_asd_genes.RDS")
casd <- unique(do.call(c, consensus_asd))
...
{
  universe <- intersect(universe, casd)
} else if (genes_to_use == "SysID")
{
  universe <- intersect(universe, id_genes)
} 
