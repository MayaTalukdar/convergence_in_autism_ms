...
all_targets <-  unname(unlist(read.table("/n/groups/walsh/indData/Maya/Finalized_Convergence_In_ASD/Cell_Lines/raw_data/process_perturbation_data_all_perturbations/final_targets_to_carry_forward.csv", header = FALSE))) #In Input_Files_Not_Generated_By_Scripts
genes_of_interest <- c(all_targets, "NTC1", "NTC2", "NTC3") #updated on 6/27/25 to not include lncRNAs
...
write.table(processed_mat_merged, "/n/groups/walsh/indData/Maya/Finalized_Convergence_In_ASD/Cell_Lines/figures/full_aracne_network_analysis/Input/matrix.txt", row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE)
