...
#should be in directory 0_Figure_1_And_S1_And_S2_And_S3/0_Figure_1C_Enrichment_Barplot_DEGs/Demo
genes <- unname(unlist(read.table("Demo_Input_Data/final_targets_to_carry_forward.csv", header = FALSE))) 
genes <- unname(sapply(genes, function(x) gsub("_", "-", x)))
path <- "Demo_Input_Data/DEG_Results/"
...
#read in SFARI genes 
sfari <- read.csv("Demo_Input_Data/sfari.csv", header = TRUE) 
SFARI_genes <- unique(sfari$gene.symbol)
all_SFARI_genes <- SFARI_genes

#read in ID genes 
id_genes <- read.csv("Demo_Input_Data/SysNDD_ID_genes_definitive.csv", header = TRUE) 
...
pdf("SFARI_gene_enrichment_barplot.pdf", height = 3, width = 5)
...
###################
cols <- "lightgray"
pdf("id_gene_enrichment_barplot.pdf", height = 3, width = 5)
...
###################
pdf("sfari_id_gene_enrichment_barplot.pdf", height = 3, width = 10)
...
write.table(merged_df, "supp_table_fig_1c.txt", sep = "\t", row.names = F, col.names = T, quote = F)